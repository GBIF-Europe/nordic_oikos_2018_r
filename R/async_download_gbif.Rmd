---
title: "Asyncronous download using the rgbif package"
author: "AGF"
date: "November 15, 2017"
output: rmarkdown::github_document
---

# Asyncronous download using the rgbif package
...
This is done in three steps. First we find a species key (for convenience, we do this step outside the request for download key) using the name_suggest function. Secondly, we request the download key by sending an API call to GBIF using the occ_download command. Then we request the download. For convenience, we here add a little [time-delay function](https://gist.githubusercontent.com/andersfi/1e7cd54cf4d12e86f0ecc66effd86129/raw/0d40d1971427aecd0c469c062c0693320392435b/download_from_GBIF_key) so that we don't have to watch for e-mails arriving from GBIF, but instead can go and have a cup of coffee. Finally, download the data. 

The current script is set up to store the downloaded data to your disk (in your R working directory). This is convenient if you work with large datasets and want to store the data for re-use in later sessions. You can alternatively download to a temporary file. 

```{r searchandgetkey, eval=FALSE}

# 1. we find a taxon key - get list of gbif keys to filter download
...
# Finally, but important - Citation 

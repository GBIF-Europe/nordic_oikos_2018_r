...
Session 6 focus on working with environment layers, mapping, cropping and masking layers using the [Raster R-package](https://cran.r-project.org/web/packages/raster/index.html) and other tools.
...
### Cut environment layer to extent (square)
```{r eval=FALSE}
library(raster)
library(rgdal)
ext <- extent(3,35,54,72) ## minLon=3, maxLon=35, minLat=54, MaxLat=72 for Scandinavia
env_cut <- crop(env, ext, snap="out", filename="./demo_data/env_cut.tif") ## square output
plot(env_cut)
#env_cut <- crop(env, ext, snap="out", filename="./demo_data/env_cut.tif") ## square output
#myraster.crop <- crop(myraster, e, snap="out", filename="myoutput.tif") ## square output
```

### Cut environment layer to mask (from a shaepfile/vector data)
```{r eval=FALSE}
data(wrld_simpl) ## here you can also read in your OWN vector data
...
## crop and mask
env_crop <- crop(env, extent(norway_mask))
env_mask <- mask(env_crop, norway_mask)
...
## ext <- extent(-180,180,-90,90) # minLon, maxLon, minLat, maxLat -- for world map
ext <- extent(3,35,54,72) ## minLon=3, maxLon=35, minLat=54, MaxLat=72 for Scandinavia
#plot.new()

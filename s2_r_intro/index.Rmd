...
The Swedish Research Council is the funder of Biodiversity Atlas Sweden - Grant No 2017-00688
...
- Before you start doing work - How can you get it all onto your laptop? 
- What is git, GitHub and how can you use [GitFlow](https://guides.github.com) to collaborate with others?

If time permits:

- Example: A function for Automated [Downloads of Event Data from GBIF.org](https://www.gbif.org/dataset/search?type=SAMPLING_EVENT)
- The path from an R script towards a function in a package?
- Example: A simple web service serving geospatial data
...
## Which packages do you need?
    ...
    # rgrep "library" | grep -oP "library\(.*?\)" | \
    #  sort -u | uniq | sed 's/library[(]\(.*\)[)]/\1/' | xargs

    rgbif rstudioapi ALA4R  # data access
    tidyr tidyverse dplyr plyr rio stringr # wrangling   
    DT gapminder plotly RColorBrewer # presentation / visuals
    spocc rgeos rgdal mapproj maps maptools raster mapr  # geospatial
    ape phytools  # phylogenetic tools
    
What packages are in a specific Docker image or on your system?
...
The `git` tool allows decentralized asynchronous collaboration across individuals and teams by managing different versions of sets of files (often code) and this tool supports a workflow, increasingly standardized and [known as "GitFlow"](https://guides.github.com/introduction/flow/), to merge so called "Pull Requests" or "Merge Request" which combine different contributed changes or branches into new master versions.

## Solo work or team work?

It can be used locally by a single user because it allows to track changes and allows reverting to earlier versions etc. It is also really useful at scale - for example when collaborating with several other colleagues and synching changes from remote repositories at [GitHub](https://github.com) where open source code can be stored and citable code can be [published](https://guides.github.com/activities/citable-code/) at no cost.
...
If you [use the CLI to push](https://help.github.com/articles/adding-an-existing-project-to-github-using-the-command-line/) HTML files to a branch you name ["gh-pages"](https://pages.github.com/), it will [appear on the Internet](https://GBIF-Europe.github.io/s2_r_intro)
...
```bash
    ...
    git remote add origin git@github.com:GBIF-Europe/nordic_oikos_2018_r.git
...
```bash
    ...
    firefox https://GBIF-Europe.github.io/s2_r_intro
